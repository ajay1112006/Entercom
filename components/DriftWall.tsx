...
const DriftWall = ({
  ...
  const renderTile = (item: DriftWallItem, id: string, colIndex: number) => {
    const inner = (
      <span className="drift-wall__inner">
        <img src={item.image} alt={item.author || item.title || ''} loading="lazy" decoding="async" draggable={false} />
        <span className="drift-wall__overlay" aria-hidden="true" />
        
        {/* Testimonial Content Overlay */}
        <div className="drift-wall__content">
          <div className="flex items-center justify-between gap-2">
            <div className="flex items-center gap-1 text-amber-400">
              {[...Array(item.rating || 5)].map((_, starIdx) => (
                <Star key={starIdx} className="w-3 h-3 fill-amber-400" />
              ))}
            </div>
            {item.company && (
              <span className="text-[10px] font-mono font-bold uppercase tracking-wider text-indigo-300 bg-indigo-500/20 px-2 py-0.5 rounded border border-indigo-500/30">
                {item.company}
              </span>
            )}
          </div>
          ...
          {item.quote && (
            <p className="text-xs text-white/90 italic font-serif leading-relaxed line-clamp-3 my-1 drop-shadow-md">
              "{item.quote}"
            </p>
          )}
          ...
          <div className="flex items-center gap-2 pt-1 border-t border-white/10">
            ...
            <div className="min-w-0">
              <span className="text-xs font-bold text-white block truncate leading-none">
                {item.author || item.title || 'Client Partner'}
              </span>
              {item.role && (
    ...
    return (
      <div key={id} tabIndex={0} role="button" aria-label={item.author || item.title || 'tile'} {...commonProps}>
        {inner}
      </div>
    );
  };
  ...
  return (
    <div
      ...
      role="group"
      aria-label="Drifting wall of client feedback tiles"
    >
