...
const DEFAULT_ITEMS: DepthCarouselItem[] = [
  { image: 'https://picsum.photos/seed/depth1/800/1000', alt: 'Slide 1', title: 'Aetheris Horizon', category: 'SaaS' },
  { image: 'https://picsum.photos/seed/depth2/800/1000', alt: 'Slide 2', title: 'PulsePay Global', category: 'Web3' },
  { image: 'https://picsum.photos/seed/depth3/800/1000', alt: 'Slide 3', title: 'Verve Atelier', category: 'E-Commerce' },
  { image: 'https://picsum.photos/seed/depth4/800/1000', alt: 'Slide 4', title: 'Kinetix Robotics', category: 'Studio' },
  { image: 'https://picsum.photos/seed/depth5/800/1000', alt: 'Slide 5', title: 'Orbital Cloud', category: 'Enterprise' }
];

const clamp = (v: number, min: number, max: number) => Math.min(Math.max(v, min), max);
const normalizeItem = (it: DepthCarouselItem) =>
  typeof it === 'string' ? { image: it, alt: '', title: '' } : it;
...
const DepthCarousel = ({
  ...
  const onCardClick = useCallback(
    (index: number) => {
      ...
      if (active === index) {
        if (onItemClick) onItemClick(data[index].rawProject || data[index]);
      } else {
        setFocus(index, true);
      }
    },
      ...
      <div className="depth-carousel__stage" ref={stageRef}>
        {data.map((item, i) => (
          ...
          >
            ...
            {/* Gradient Overlay for Project Text */}
            <span className="depth-carousel__overlay" aria-hidden="true" />

            {/* Project Card Text HUD Content */}
            <div className="depth-carousel__info flex flex-col justify-end space-y-2">
              {item.category && (
                <div className="flex items-center justify-between gap-2">
                  <span className="text-[10px] font-mono font-bold uppercase tracking-wider text-indigo-400 bg-indigo-500/20 px-2 py-0.5 rounded border border-indigo-500/30">
                    {item.category}
                  </span>
                  <span className="w-7 h-7 rounded-full bg-white/10 backdrop-blur-md flex items-center justify-center text-white opacity-0 group-hover:opacity-100 transition-opacity">
